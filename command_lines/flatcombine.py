...
def flat(files: List[str], output: str, override: bool, ope: str, rej: str, discrete: str, scale: str):
    """
    flatcombine -- Combine and process flat field images

    :param files: List of flat field images to  combine.
    :param output: Output flat field root image name.
    ...
    :param discrete: Combine images by subset parameter? If True then the input images are grouped.
    ...
    if discrete is not None:
        ...
        co = Combine(fa)
        _ = co.flatcombine(ope, output=output, override=override, reject=rej, scale=scale)
